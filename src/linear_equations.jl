...
给定 ``n`` 阶线性方程组 ``Ax = b``, 设 ``A = (a_{ij}), b = (b_{i})``, 其中 ``1 \leqslant i, j \leqslant n`` 且 ``A`` 的对角元都不为 ``0``. ``D, L, U`` 分别为 ``A`` 的主对角, 上三角和下三角部分, 即:
...
``x^{(k)}, x^{(k + 1)}`` 分别为第 ``k`` 次及第 ``k + 1`` 次迭代后得到的值, 则 Jacobi 迭代法的迭代公式为:

```math
\begin{cases}
    x_1^{(k + 1)} & = (- a_{12} x_{2}^{(k)} - a_{13} x_{3}^{(k)} \cdots - a_{1n} x_{n}^{(k)}) / a_{11} + b_{1} / a_{11} \\
    x_2^{(k + 1)} & = (- a_{21} x_{1}^{(k)} - a_{23} x_{3}^{(k)} \cdots - a_{2n} x_{n}^{(k)}) / a_{22} + b_{2} / a_{22} \\
                  & \vdots \\
    x_n^{(k + 1)} & = (- a_{n1} x_{1}^{(k)} - a_{n2} x_{2}^{(k)} \cdots - a_{nn - 1} x^{(k)}) / a_{nn} + b_{n} / a_{nn} \\
\end{cases}
```
...
``x^{(k)}, x^{(k + 1)}`` 分别为第 ``k`` 次及第 ``k + 1`` 次迭代后得到的值, 则 GaussSeidel 迭代法的迭代公式为:

```math
\begin{cases}
    x_1^{(k + 1)} & = (- a_{12} x_{2}^{(k)} - a_{13} x_{3}^{(k)} \cdots - a_{1n} x_{n}^{(k)}) / a_{11} + b_{1} / a_{11} \\
    x_2^{(k + 1)} & = (- a_{21} x_{1}^{(k + 1)} - a_{23} x_{3}^{(k)} \cdots - a_{2n} x_{n}^{(k)}) / a_{22} + b_{2} / a_{22} \\
                  & \vdots \\
    x_n^{(k + 1)} & = (- a_{n1} x_{1}^{(k + 1)} - a_{n2} x_{2}^{(k + 1)} \cdots - a_{nn - 1} x^{(k + 1)}) / a_{nn} + b_{n} / a_{nn} \\
\end{cases}
```
...
``x^{(k)}, x^{(k + 1)}`` 分别为第 ``k`` 次及第 ``k + 1`` 次迭代后得到的值, ``\tilde{x}^{(k + 1)}`` 是 [`GaussSeidel`](@ref) 法定义的中间变量, ``\omega`` 为松弛因子, 则 SOR 迭代法的迭代公式为:

```math
\begin{cases}
    \tilde{x}_1^{(k + 1)} & = (- a_{12} x_{2}^{(k)} - a_{13} x_{3}^{(k)} \cdots - a_{1n} x_{n}^{(k)}) / a_{11} + b_{1} / a_{11} \\
    \tilde{x}_2^{(k + 1)} & = (- a_{21} x_{1}^{(k + 1)} - a_{23} x_{3}^{(k)} \cdots - a_{2n} x_{n}^{(k)}) / a_{22} + b_{2} / a_{22} \\
                      & \vdots \\
    \tilde{x}_n^{(k + 1)} & = (- a_{n1} x_{1}^{(k + 1)} - a_{n2} x_{2}^{(k + 1)} \cdots - a_{nn - 1} x^{(k + 1)}) / a_{nn} + b_{n} / a_{nn} \\
...
"""
    multiple_itr(itr, x₀, args...; n=1000, ϵ=nothing, kwargs...)
